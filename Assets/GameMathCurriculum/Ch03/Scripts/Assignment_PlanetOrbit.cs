...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        Vector3 translation = new Vector3(0f, 0f, planetOrbitRadius);
        Quaternion angle = Quaternion.Euler(0f, Time.time * planetOrbitSpeed, 0f);
        Matrix4x4 matrix = Matrix4x4.TRS(orbitCenter, angle, Vector3.one);
        planetWorldPos = matrix.MultiplyPoint3x4(translation);


        translation = new Vector3(0f, 0f, satelliteOrbitRadius);
        angle = Quaternion.Euler(0f, Time.time * satelliteOrbitSpeed, 0f);
        matrix = Matrix4x4.TRS(planetWorldPos, angle, Vector3.one);
        satelliteWorldPos = matrix.MultiplyPoint3x4(translation);
