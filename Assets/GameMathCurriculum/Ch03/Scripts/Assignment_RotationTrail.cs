...
public class Assignment_RotationTrail : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!autoRotate) { UpdateUI(); return; }

        Vector3 translation = new Vector3(armLength, 0f, 0f);
        Quaternion angle = Quaternion.Euler(0f, rotationSpeed * rotationAngle * Time.time, 0f);
        Matrix4x4 matrix = Matrix4x4.TRS(Vector3.zero, angle, Vector3.one);

        transform.position = matrix.MultiplyPoint(translation);

        if (trailPositions.Count >= trailLength)
        {
            trailPositions.Remove(trailPositions[0]);
        }
