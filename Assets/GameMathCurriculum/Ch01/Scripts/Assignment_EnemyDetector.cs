...
public class Assignment_EnemyDetector : MonoBehaviour
{
    ...
    private bool IsDetected(Transform enemy)
    {
        ...
        if (detectionRange < distance.magnitude) return false;

        Vector3 distanceNorm = distance.normalized;
        float angleCos = Vector3.Dot(transform.forward, distanceNorm);
        float angle = Mathf.Acos(angleCos) * Mathf.Rad2Deg;

        if (angle > detectionFOV / 2f) return false;
