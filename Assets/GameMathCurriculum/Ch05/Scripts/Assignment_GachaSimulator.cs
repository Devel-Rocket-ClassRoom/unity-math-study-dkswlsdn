...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    private void ExecutePull()
    {
        float additionalRate =
            Mathf.Max((1f - baseRate) / (hardPity - softPityStart + 1) * (currentPityCount - softPityStart + 1), 0);

        float randomValue = Random.value;

        currentEffectiveRate = baseRate + additionalRate;


        if (baseRate + additionalRate >= randomValue)
        {
        ...
        {
            pullHistory.RemoveRange(0, pullHistory.Count - MAX_HISTORY);
        }
    }
    ...
    private void UpdateUI()
    {
        ...
        if (ssrPityList.Count > 0)
        {
            ...
            expectedMsg += $"최근 3회: ";
            for (int i = ssrPityList.Count - 1; i >= Mathf.Max(ssrPityList.Count - 3, 0); i--)
            {
