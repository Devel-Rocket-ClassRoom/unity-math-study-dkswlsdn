...
public class Assignment_PatrolGuard : MonoBehaviour
{
    ...
    private void Start()
    {
        transform.rotation = Quaternion.LookRotation(waypoints[0].position);
    }
    ...
    private void Update()
    {
        ...
        Quaternion dirToNextQuat = Quaternion.LookRotation(dirToNext, Vector3.up);

        transform.rotation = Quaternion.RotateTowards(transform.rotation, dirToNextQuat, turnSpeed * Time.deltaTime);
