...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            float angle = Mathf.Lerp(0f, 360f, (float)i / currentUnitCount);
            units[i].position = leader.position + Quaternion.AngleAxis(angle, leader.transform.up)
                * leader.transform.forward * formationRadius;

            Vector3 toTarget = (units[i].position - leader.position).normalized;
            units[i].forward = toTarget;
        }
