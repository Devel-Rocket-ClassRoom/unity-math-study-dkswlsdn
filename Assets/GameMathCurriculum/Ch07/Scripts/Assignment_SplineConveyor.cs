...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        globalT = Mathf.Repeat(globalT, cycleDuration);
        currentSpeedMultiplier = speedCurve.Evaluate(globalT / cycleDuration);

        for (int i = 0; i < boxes.Length; i++)
        {
            float localT = globalT * currentSpeedMultiplier - (float)i / boxes.Length;
            boxes[i].position = EvaluateSpline(waypoints, Mathf.Repeat(localT, 1f));
        }
